export default class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(w / 2, h / 2 - 20, 'Core Gameplay Prototype', {
      fontSize: '18px', color: '#aaaaaa', fontFamily: 'monospace'
    }).setOrigin(0.5);

    this.add.text(w / 2, h / 2 + 20, 'Arrow keys / touch to move  ·  T to switch time', {
      fontSize: '13px', color: '#666666', fontFamily: 'monospace'
    }).setOrigin(0.5);

    this.createMenuButton(w / 2, h / 2 + 80, 'START', () => {
  ...
  createMenuButton(x, y, label, callback) {
    ...
    draw();
    this.add.text(x, y, label, { fontSize: '24px', color: '#ffffff', fontFamily: 'monospace' }).setOrigin(0.5);
